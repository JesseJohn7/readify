...
export default function GeneratePage() {
  ...
  async function handleCommit() {
    ...
    try {
      ...
      setCommitMsg(sha ? "README.md updated on GitHub ✓" : "README.md committed to GitHub ✓");
    } catch (e: any) {
      setCommitStatus("error");
      setCommitMsg(e.message || "Commit failed. Try again.");
    } finally {
      setCommitting(false);
    }
  }
        ...
        <aside className="lg:w-64 shrink-0 px-6 lg:px-0 mb-6 lg:mb-0 lg:pr-6">
          <div className="bg-gray-900 rounded-xl p-4 border border-gray-800">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-sm font-medium text-white truncate">{data.repo.name}</span>
            </div>
            {data.repo.description && (
              <p className="text-xs text-gray-400 mb-3 leading-relaxed">{data.repo.description}</p>
            )}
